...
from keibadb.kdb.models import Horse
...
def create_horse(horse_key, horse_name="", depth=0, is_stallion=False):
    ...
    print(
        "  " * depth
        + f"horse_key={horse_key},father={father},father_name={father_name},mother={mother},mother_name={mother_name},trainer_name={trainer_name},traner_key={trainer_key},training_center={training_center},owner_name={owner_name},owner_key={owner_key},farm_name={farm_name},farm_key={farm_key}"
    )
    ...
    if Horse.objects.filter(horse_key=horse_key).exists():
        if is_active:
            ...
            Horse.objects.filter(horse_key=horse_key).update(
                name=horse_name,
